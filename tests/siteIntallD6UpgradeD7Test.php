 ...
 * @group commands
 */
class siteInstallD6UpgradeD7Case extends CommandUnishTestCase {
  ...
  public function testExtraConfigurationOptions() {
    ...
    $sites = $this->setUpDrupal(1, TRUE, '6', 'default', array('source'));
    $root = $this->webroot();
    $site = key($sites);
...
*/
    ...
    // Set up a target codebase without installing Drupal.
    $sites2 = $this->setUpDrupal(1, TRUE, 7, 'testing', array('target'));
    $root2 = $this->webroot();
    $site2 = key($sites);

    $this->drush('sql-sync', array('@source', '@target'));
    $this->drush('updatedb', '@target');
